...
async function fetchActiveTrips() {
	try {
		const res = await fetch("/api/trips/active-trips");
		...
		if (data.activeTrips) {
			data.activeTrips.forEach(async (trip) => {
				...
				const pickupRouteRes = await fetch(
					`/api/trips/get-directions?origin=${lat},${lng}&destination=${originLat},${originLng}`,
					{
						headers: {
				...
				const routeRes = await fetch(
					`/api/trips/get-directions?origin=${originLat},${originLng}&destination=${destLat},${destLng}`,
					{
						headers: {
							Authorization: `Bearer ${localStorage.getItem(
								"adminToken"
